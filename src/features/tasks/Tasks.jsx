...
const INITIAL_TASKS = [
  {
    ...
    description:
      "Create a modern, glassmorphism hero section for the new marketing site.",
    status: "In Progress",
    priority: "High",
    assigneeId: "u1",
    dueDate: "2026-05-10",
  },
  {
    ...
    description:
      "Replace all legacy components with standardized Shadcn UI primitives.",
    status: "Done",
    priority: "High",
    assigneeId: "u2",
    dueDate: "2026-05-02",
  ...
  },
  {
    ...
    description:
      "Review and optimize the slow-performing queries on the team page.",
    status: "In Review",
    priority: "Medium",
    assigneeId: "u4",
    dueDate: "2026-05-08",
  ...
  },
  {
    ...
    description:
      "The hamburger menu does not close when a link is clicked on iOS Safari.",
    status: "In Progress",
    priority: "High",
    assigneeId: "u3",
    dueDate: "2026-05-06",
...
function Tasks() {
  ...
  const handleTaskSubmit = (values) => {
    ...
    if (values.id) {
      setTasks((cur) =>
        cur.map((t) => (t.id === values.id ? { ...t, ...payload } : t))
      );
    } else {
  ...
  const changeTaskStatus = (id, newStatus) => {
    setTasks((cur) =>
      cur.map((t) => (t.id === id ? { ...t, status: newStatus } : t))
    );
  };
    ...
    <main className="flex w-full min-w-0 animate-in flex-col gap-6 duration-500 fade-in">
      ...
      >
        ...
        <TabsContent value="board" className="mt-0 w-full min-w-0 outline-none">
          <div className="grid grid-cols-1 gap-4 pt-2 pb-4 md:grid-cols-2 xl:grid-cols-4">
            {STATUS_COLUMNS.map((statusName) => {
              ...
              return (
                <div
                  key={statusName}
                  className="flex flex-col gap-3 rounded-2xl border border-border/40 bg-muted/20 p-3 shadow-sm backdrop-blur-md transition-all"
                >
                  {/* Column Header */}
                  <div className="flex items-center justify-between px-1 pb-1">
                    <div className="flex items-center gap-2.5">
                      <div className="flex size-7 items-center justify-center rounded-lg border border-background/50 bg-background/50 shadow-sm backdrop-blur-sm">
                        <StatusIcon
                          className={cn("size-4", STATUS_COLORS[statusName])}
                        />
                      </div>
                    ...
                    <Badge
                      variant="secondary"
                      className="h-5 rounded-md border-border/50 bg-background/50 px-1.5 text-[10px] font-bold text-muted-foreground shadow-sm"
                    >
                      {columnTasks.length}
                    </Badge>
                  </div>
                  ...
                  <div className="flex min-h-[150px] flex-col gap-3">
                    {columnTasks.map((task) => (
                      <Card
                        key={task.id}
                        className="group relative overflow-hidden rounded-xl border-border/50 bg-background/80 shadow-sm backdrop-blur transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md"
                      >
                        <CardContent className="p-3.5 sm:p-4">
                          ...
                          <h4 className="mb-1.5 text-sm leading-tight font-semibold">
                            {task.title}
                          </h4>
                          ...
                          <div className="flex items-center justify-between border-t border-border/40 pt-3">
                            ...
                            {task.assigneeId && usersById[task.assigneeId] ? (
                              <UserAvatar
                                name={usersById[task.assigneeId].name}
                                size="sm"
                              />
                            ) : (
                ...
                <TableHeader>
                  <TableRow className="border-border/40 bg-muted/30 hover:bg-muted/30">
                    <TableHead className="w-[35%]">
                      <TableHeadLabel Icon={IconChecklist} label="Task" />
                    </TableHead>
                    <TableHead>
                      <TableHeadLabel Icon={IconCircleCheck} label="Status" />
                    </TableHead>
                    <TableHead>
                      <TableHeadLabel Icon={IconBolt} label="Priority" />
                    </TableHead>
                    <TableHead>
                      <TableHeadLabel Icon={IconCalendar} label="Due Date" />
                    </TableHead>
                    <TableHead>
                      <TableHeadLabel Icon={IconCheck} label="Assignee" />
                    </TableHead>
                      ...
                      >
                        ...
                        <TableCell>
                          <Badge
                            variant="outline"
                            className={cn(
                              "px-2 py-0 text-[10px] tracking-wider uppercase",
                              PRIORITY_STYLES[task.priority]
                            )}
                          >
                            {task.priority}
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <Form {...taskForm}>
            ...
            >
              ...
              <div className="mt-2 flex justify-end gap-3 border-t border-border/40 pt-5">
                <Button
                  type="button"
                  variant="ghost"
